...
from .models import Employee, Department, Position, News, BonusTransfer, Holiday, StaffMember, Notification, SystemSettings, SupportRequest
from .forms import EmployeeRegistrationForm, EmployeeLoginForm, BonusTransferForm, ProfileEditForm, NewsForm, StaffMemberForm, ProfilePasswordChangeForm, SupportRequestCreateForm, SupportRequestAdminUpdateForm
from django.utils import timezone
...
def _rating_rows_for_period(start_date, end_date):
    """
    Рейтинг получателей за период: все с participates_in_bonus (включая неактивных
    зарегистрированных) + переводы без учётной записи получателя (агрегат по to_display_name).
    """
    emp_qs = Employee.objects.filter(participates_in_bonus=True).annotate(
        total_received=Sum(
            'received_transfers__amount',
            filter=Q(
                received_transfers__created_at__date__gte=start_date,
                received_transfers__created_at__date__lte=end_date,
                received_transfers__is_deleted=False,
            )
        )
    ).select_related('department').filter(total_received__isnull=False)

    rows = []
    for emp in emp_qs:
        tr = emp.total_received
        ...
        rows.append({
            ...
            'total': float(tr),
            'not_in_system': not emp.is_active,
        })

    orphan = BonusTransfer.objects.filter(
        to_employee__isnull=True,
        is_deleted=False,
        created_at__date__gte=start_date,
        created_at__date__lte=end_date,
    ).exclude(to_display_name='').values('to_display_name').annotate(total=Sum('amount'))
...
def home_view(request):
    ...
    # Рейтинг за текущий месяц (включая неактивных получателей и переводы без учётки)
    now = timezone.now()
    current_month = now.month
    last_day = monthrange(now.year, now.month)[1]
    month_start = date(now.year, now.month, 1)
    month_end = date(now.year, now.month, last_day)
    rating = _rating_rows_for_period(month_start, month_end)[:10]
...
def rating_view(request):
    ...
    # Рейтинг: участники премирования (в т.ч. неактивные) + переводы без учётной записи получателя
    rating_rows = _rating_rows_for_period(start_date, end_date)
    top10 = rating_rows[:10]
    chart_data = {
        'labels': [r['name'] for r in top10],
        'data': [r['total'] for r in top10],
    }
...
def bonus_transfer_view(request):
    ...
    if request.method == 'POST':
            ...
            try:
                ...
                if staff.employee_profile:
                    ...
                    try:
                        ...
                        try:
                            default_monthly = SystemSettings.get_settings().monthly_bonus_amount
                        except Exception:
                            from decimal import Decimal
                            default_monthly = Decimal('1000.00')
                        ...
                        to_employee = Employee.objects.create(
                            ...
                            monthly_bonus_balance=default_monthly,
                            received_bonus_balance=0,
                        )
                        to_employee.set_unusable_password()
                        to_employee.save(update_fields=['password'])
        ...
        if to_employee.id == request.user.id:
            messages.error(request, 'Нельзя переводить бонусные рубли самому себе.')
            return redirect('bonus_transfer')
...
def profile_view(request):
    if request.method == 'POST':
        ...
        if form_type == 'password':
            password_form = ProfilePasswordChangeForm(request.POST, user=request.user)
...
def notifications_view(request):
    ...
    # Помечаем все как прочитанные при явном запросе (GET)
    if request.method == 'GET' and 'mark_read' in request.GET:
        Notification.objects.filter(user=request.user, is_read=False).update(is_read=True)
        return redirect('notifications')
    
    # Удаление всех по GET (как ссылка из меню с подтверждением)
    if request.method == 'GET' and request.GET.get('clear_all'):
...
@login_required
def support_help_view(request):
    """Страница помощи сотрудника: инструкция, создание и просмотр своих обращений."""
    if request.user.is_admin:
        return redirect('admin_support_requests')

    if request.method == 'POST':
        form = SupportRequestCreateForm(request.POST)
        if form.is_valid():
            support_request = form.save(commit=False)
            support_request.employee = request.user
            support_request.save()

            # Уведомляем всех администраторов о новом обращении
            admins = Employee.objects.filter(is_admin=True).exclude(id=request.user.id)
            for admin in admins:
                Notification.objects.create(
                    user=admin,
                    type='system',
                    title='Новое обращение сотрудника',
                    message=f'{request.user.get_full_name()} создал(а) обращение #{support_request.id}.',
                )

            messages.success(request, 'Обращение отправлено администраторам.')
            return redirect('support_help')
    else:
        form = SupportRequestCreateForm()

    my_requests = SupportRequest.objects.filter(employee=request.user).select_related('handled_by')
    context = {
        'form': form,
        'my_requests': my_requests,
    }
    return render(request, 'employees/support_help.html', context)


@login_required
def admin_support_requests_view(request):
    """Вкладка обращений для администратора."""
    if not request.user.is_admin:
        messages.error(request, 'У вас нет прав доступа')
        return redirect('home')

    status_filter = request.GET.get('status', 'all')
    requests_qs = SupportRequest.objects.select_related('employee', 'handled_by')
    if status_filter in {SupportRequest.STATUS_NEW, SupportRequest.STATUS_IN_PROGRESS, SupportRequest.STATUS_CLOSED}:
        requests_qs = requests_qs.filter(status=status_filter)

    status_choices = [
        ('all', 'Все'),
        (SupportRequest.STATUS_NEW, 'Новое'),
        (SupportRequest.STATUS_IN_PROGRESS, 'В работе'),
        (SupportRequest.STATUS_CLOSED, 'Закрыто'),
    ]
    context = {
    ...
    return render(request, 'employees/admin_support_requests.html', context)


@login_required
def admin_support_request_update_view(request, request_id):
    """Изменение статуса обращения и ответ администратора."""
    if not request.user.is_admin:
        messages.error(request, 'У вас нет прав доступа')
        return redirect('home')
    ...
    form = SupportRequestAdminUpdateForm(request.POST, instance=support_request)
    if form.is_valid():
        ...
        Notification.objects.create(
            user=support_request.employee,
            type='system',
            title='Обновлен статус обращения',
            message=(
                f'Ваше обращение #{support_request.id}: статус «{updated_request.get_status_display()}». '
                f'Ответ администратора: {updated_request.admin_response or "без комментария"}'
            ),
        )
        messages.success(request, f'Обращение #{support_request.id} обновлено.')
    else:
        messages.error(request, 'Не удалось обновить обращение. Проверьте заполнение полей.')

    next_status = request.GET.get('status') or 'all'
    return redirect(f"{reverse('admin_support_requests')}?status={next_status}")
...
@login_required
def admin_user_reset_password_view(request, employee_id):
    ...
    messages.success(
        request,
        f'Пароль для {employee.get_full_name()} сброшен. Временный пароль: {temp_password}'
    )
...
def admin_transfer_delete_view(request, transfer_id):
    ...
    if request.method == 'POST':
        ...
        if not cancellation_reason:
            messages.error(request, 'Укажите причину отмены перевода.')
            return render(request, 'employees/admin_transfer_delete.html', {'transfer': transfer})

        # Возвращаем средства (только если учётная запись ещё существует)
        ...
        transfer.cancellation_reason = cancellation_reason
        transfer.save()
        ...
        if transfer.to_employee_id:
            Notification.objects.create(
                ...
                message=f'Перевод на сумму {transfer.amount} руб. от {transfer.get_from_name()} был отменен администратором. Причина: {cancellation_reason}',
                related_transfer=transfer
            )
        ...
        if transfer.from_employee_id:
            Notification.objects.create(
                ...
                type='transfer_cancelled',
                title='Перевод отменен',
                message=f'Ваш перевод на сумму {transfer.amount} руб. для {transfer.get_to_name()} был отменен администратором. Причина: {cancellation_reason}. Средства возвращены на ваш баланс.',
                related_transfer=transfer
            )
...
def admin_settings_view(request):
    ...
    if request.method == 'POST':
        ...
        if form_type == 'bonus':
            ...
            if settings.company_logo:
                ...
                settings.save()
                messages.success(request, 'Логотип компании удалён.')
            else:
                messages.info(request, 'Логотип не был установлен.')
            ...
            try:
                ...
                dn = dept.name
                dept.delete()
                messages.success(request, f'Отдел «{dn}» удалён.')
            except Department.DoesNotExist:
                messages.error(request, 'Отдел не найден.')
        
        elif form_type == 'add_position':
            name = (request.POST.get('position_name') or '').strip()
            dept_id = request.POST.get('position_department_id') or None
            if not name:
                messages.error(request, 'Введите название должности.')
            else:
                dept = None
                if dept_id:
                    try:
                ...
                Position.objects.create(name=name, department=dept)
                messages.success(request, f'Должность «{name}» добавлена.')
        
        elif form_type == 'delete_position':
            pos_id = request.POST.get('position_id')
            try:
                pos = Position.objects.get(pk=pos_id)
                pn = pos.name
                pos.delete()
                messages.success(request, f'Должность «{pn}» удалена.')
    ...
    departments = Department.objects.all().order_by('name')
    positions = Position.objects.select_related('department').order_by('department__name', 'name')
    
    context = {
        'settings': settings,
        'departments_admin': departments,
        'positions_admin': positions,
    }
...
def admin_export_transfers_view(request):
    """Экспорт реестра премий с фильтрами периода и сотрудников."""
    if not request.user.is_admin:
        messages.error(request, 'У вас нет прав доступа')
        return redirect('home')
    ...
    period_value = request.GET.get('period_value')
    start_date, end_date, period_label, normalized_period_value = _resolve_export_period(period_type, period_value)

    employee_mode = request.GET.get('employee_mode', 'all')
    selected_employee_ids = [employee_id for employee_id in request.GET.getlist('employees') if employee_id.isdigit()]

    transfers = BonusTransfer.objects.filter(
        created_at__date__gte=start_date,
        created_at__date__lte=end_date,
        is_deleted=False,
    ).select_related('to_employee', 'to_employee__department', 'to_employee__position').order_by(
        'to_display_name', 'to_employee__last_name'
    )

    if employee_mode == 'selected' and selected_employee_ids:
        transfers = transfers.filter(to_employee_id__in=selected_employee_ids)

    if request.GET.get('download') == '1':
        export_format = request.GET.get('format', 'excel')
        file_period_label = _period_label_for_filename(period_type, period_value, normalized_period_value)
        if export_format == 'pdf':
            return _export_transfers_pdf(transfers, period_label, file_period_label)
        return _export_transfers_excel(transfers, period_label, file_period_label)
    ...
    for i in range(12):
        ...
        months.append({
            'value': date_obj.strftime('%Y-%m'),
            'label': f"{MONTH_NAMES_RU[date_obj.month]} {date_obj.year}",
        })
    ...
    employees = Employee.objects.filter(participates_in_bonus=True).order_by('last_name', 'first_name')

    context = {
        'period_type': period_type,
        'period_value': normalized_period_value,
        'period_label': period_label,
        'months': months,
        'quarters': quarters,
        'years': years,
    ...
    return render(request, 'employees/admin_export_transfers.html', context)


def _resolve_export_period(period_type, period_value):
    """Определяет диапазон дат и подпись для экспорта."""
    now = timezone.now()
    current_year = now.year
    current_month = now.month
    current_quarter = (current_month - 1) // 3 + 1
    ...
    if period_type == 'year':
        ...
        start_date = date(year, 1, 1)
        end_date = date(year, 12, 31)
        return start_date, end_date, f'{year} год', str(year)
    ...
    if period_type == 'quarter':
        ...
        return start_date, end_date, f'{quarter} квартал {year}', f'{year}-Q{quarter}'
    ...
    return start_date, end_date, f'{MONTH_NAMES_RU[month]} {year}', f'{year}-{month:02d}'


def _period_label_for_filename(period_type, raw_period_value, normalized_period_value):
    """Человекочитаемый период для имени файла."""
    if period_type == 'year':
        return f'{normalized_period_value} год'
    if period_type == 'quarter':
        year, quarter = raw_period_value.split('-Q') if raw_period_value else normalized_period_value.split('-Q')
        return f'{quarter} квартал {year}'
    year, month = map(int, normalized_period_value.split('-'))
    return f'{MONTH_NAMES_RU[month]} {year}'


def _transfers_export_attachment_filename(period_label, ext):
    """Имя файла вложения для реестра начислений."""
    base = f'Начисление из ГП за {period_label}{ext}'
    return f"attachment; filename*=UTF-8''{quote(base)}"


def _export_transfers_excel(transfers, period_label, file_period_label):
    """Экспорт в Excel"""
    # Создаем Excel файл
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = 'Реестр премий'
    
    # Шапка
    ws.merge_cells('A1:D1')
    ws['A1'] = f'Начисление из горизонтального премирования за "{period_label.upper()}"'
    ...
    for transfer in transfers:
        ...
        if employee:
            ...
            ws.cell(row=row, column=1, value=transfer.to_display_name or '—')
    ...
    response['Content-Disposition'] = _transfers_export_attachment_filename(
        file_period_label, '.xlsx'
    )
    wb.save(response)
...
def _export_transfers_pdf(transfers, period_label, file_period_label):
    """Экспорт в PDF с поддержкой кириллицы"""
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = _transfers_export_attachment_filename(
        file_period_label, '.pdf'
    )
    ...
    # Заголовок
    title_text = f'Начисление из горизонтального премирования за "{period_label.upper()}"'
    ...
    for transfer in transfers:
        ...
        if employee:
            ...
            full_name = transfer.to_display_name or '—'
            dept_name = ''
            pos_name = ''
