...
from .models import Employee, Department, Position, BonusTransfer, News, StaffMember, SupportRequest
...
class EmployeeRegistrationForm(UserCreationForm):
    ...
    def save(self, commit=True):
        ...
        if existing_inactive_employee:
            ...
            # Фото: из формы или из справочника сотрудников
            ...
            elif staff_member and staff_member.photo:
            ...
            if staff_member:
                ...
                staff_member.phone = user.phone
                staff_member.save(update_fields=['employee_profile', 'email', 'phone'])
            
            return existing_inactive_employee
        else:
            # Новый пользователь: фото и связь со справочником
            if commit:
                user.save()
                staff_member = StaffMember.objects.filter(
                    last_name=user.last_name,
                ...
                if staff_member:
                    if staff_member.photo and not user.photo:
                        user.photo = staff_member.photo
                        user.save(update_fields=['photo'])
                    staff_member.employee_profile = user
                    staff_member.email = user.email
                    staff_member.phone = user.phone
                    staff_member.save(update_fields=['employee_profile', 'email', 'phone'])
            return user
...
class BonusTransferForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        # Получатель может быть ещё не зарегистрирован (is_active=False), но уже в базе после перевода
    ...
    def clean(self):
        ...
        if self.from_employee:
            ...
            if to_employee and self.from_employee.id == to_employee.id:
                raise forms.ValidationError('Нельзя переводить бонусные рубли самому себе.')
...
class SupportRequestCreateForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'description': forms.Textarea(
                attrs={
                    'class': 'form-control',
                    'rows': 5,
                    'placeholder': 'Опишите, что произошло и какая помощь нужна...'
                }
            ),
        }


class SupportRequestAdminUpdateForm(forms.ModelForm):
    class Meta:
        model = SupportRequest
        ...
        widgets = {
            ...
            'admin_response': forms.Textarea(
                attrs={
                    'class': 'form-control',
                    'rows': 4,
                    'placeholder': 'Опишите, как сотруднику решить проблему...'
                }
            ),
        }
